//
// MyPrefixProductsMasterViewController.swift
// ESPMReminders
//
...
class MyPrefixProductsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var myServiceClass: MyPrefixMyServiceClass<OnlineODataProvider> {
        return self.appDelegate.myServiceClass
    }

    private var entities: [MyPrefixProduct] = [MyPrefixProduct]()
    private let logger = Logger.shared(named: "MyPrefixProductMasterViewConterollerLogger")
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.myServiceClass.fetchProducts(matching: query) { products, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MyPrefixProductsDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! MyPrefixProductsDetailViewController
