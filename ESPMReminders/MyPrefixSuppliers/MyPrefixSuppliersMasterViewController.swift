//
// MyPrefixSuppliersMasterViewController.swift
// ESPMReminders
//
...
class MyPrefixSuppliersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var myServiceClass: MyPrefixMyServiceClass<OnlineODataProvider> {
        return self.appDelegate.myServiceClass
    }

    private var entities: [MyPrefixSupplier] = [MyPrefixSupplier]()
    private let logger = Logger.shared(named: "MyPrefixSupplierMasterViewConterollerLogger")
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.myServiceClass.fetchSuppliers(matching: query) { suppliers, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MyPrefixSuppliersDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! MyPrefixSuppliersDetailViewController
