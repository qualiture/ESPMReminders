//
// MyPrefixSalesOrderItemsMasterViewController.swift
// ESPMReminders
//
...
class MyPrefixSalesOrderItemsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var myServiceClass: MyPrefixMyServiceClass<OnlineODataProvider> {
        return self.appDelegate.myServiceClass
    }

    private var entities: [MyPrefixSalesOrderItem] = [MyPrefixSalesOrderItem]()
    private let logger = Logger.shared(named: "MyPrefixSalesOrderItemMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ItemNumber", value: "\(salesorderitem.itemNumber!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.myServiceClass.fetchSalesOrderItems(matching: query) { salesOrderItems, error in
            guard let salesOrderItems = salesOrderItems else {
                completionHandler(error!)
                return
            }
            self.entities = salesOrderItems
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MyPrefixSalesOrderItemsDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! MyPrefixSalesOrderItemsDetailViewController
