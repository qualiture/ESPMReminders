//
// MyPrefixSalesOrderHeadersMasterViewController.swift
// ESPMReminders
//
...
class MyPrefixSalesOrderHeadersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var myServiceClass: MyPrefixMyServiceClass<OnlineODataProvider> {
        return self.appDelegate.myServiceClass
    }

    private var entities: [MyPrefixSalesOrderHeader] = [MyPrefixSalesOrderHeader]()
    private let logger = Logger.shared(named: "MyPrefixSalesOrderHeaderMasterViewConterollerLogger")
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.myServiceClass.fetchSalesOrderHeaders(matching: query) { salesOrderHeaders, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MyPrefixSalesOrderHeadersDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! MyPrefixSalesOrderHeadersDetailViewController
