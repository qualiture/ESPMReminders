//
// MyPrefixPurchaseOrderHeadersMasterViewController.swift
// ESPMReminders
//
...
class MyPrefixPurchaseOrderHeadersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var myServiceClass: MyPrefixMyServiceClass<OnlineODataProvider> {
        return self.appDelegate.myServiceClass
    }

    private var entities: [MyPrefixPurchaseOrderHeader] = [MyPrefixPurchaseOrderHeader]()
    private let logger = Logger.shared(named: "MyPrefixPurchaseOrderHeaderMasterViewConterollerLogger")
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.myServiceClass.fetchPurchaseOrderHeaders(matching: query) { purchaseOrderHeaders, error in
            guard let purchaseOrderHeaders = purchaseOrderHeaders else {
                completionHandler(error!)
                return
            }
            self.entities = purchaseOrderHeaders
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MyPrefixPurchaseOrderHeadersDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! MyPrefixPurchaseOrderHeadersDetailViewController
