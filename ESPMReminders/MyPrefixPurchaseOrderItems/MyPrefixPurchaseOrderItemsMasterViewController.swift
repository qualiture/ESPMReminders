//
// MyPrefixPurchaseOrderItemsMasterViewController.swift
// ESPMReminders
//
...
class MyPrefixPurchaseOrderItemsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var myServiceClass: MyPrefixMyServiceClass<OnlineODataProvider> {
        return self.appDelegate.myServiceClass
    }

    private var entities: [MyPrefixPurchaseOrderItem] = [MyPrefixPurchaseOrderItem]()
    private let logger = Logger.shared(named: "MyPrefixPurchaseOrderItemMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ItemNumber", value: "\(purchaseorderitem.itemNumber!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.myServiceClass.fetchPurchaseOrderItems(matching: query) { purchaseOrderItems, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MyPrefixPurchaseOrderItemsDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! MyPrefixPurchaseOrderItemsDetailViewController
