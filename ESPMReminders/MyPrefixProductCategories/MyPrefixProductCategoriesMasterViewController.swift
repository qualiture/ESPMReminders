//
// MyPrefixProductCategoriesMasterViewController.swift
// ESPMReminders
//
...
class MyPrefixProductCategoriesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var myServiceClass: MyPrefixMyServiceClass<OnlineODataProvider> {
        return self.appDelegate.myServiceClass
    }

    private var entities: [MyPrefixProductCategory] = [MyPrefixProductCategory]()
    private let logger = Logger.shared(named: "MyPrefixProductCategoryMasterViewConterollerLogger")
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.myServiceClass.fetchProductCategories(matching: query) { productCategories, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MyPrefixProductCategoriesDetailViewController
